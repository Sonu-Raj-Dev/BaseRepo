using Lead_Management.Core;
using Lead_Management.Core.Helper;
using Lead_Management.Service.User;
using Lead_Management.Web.Models;
    ...
    public class LoginController : BaseController
    {
        ...
        public async Task<IActionResult> Logout()
        {
            try
            {
                ...
                {
                    // Auth cookie
                    Response.Cookies.Delete("LeadCookie", new CookieOptions { Path = "/Lead", HttpOnly = true, Secure = true, SameSite = SameSiteMode.None });
                    Response.Cookies.Append("LeadCookie", "", new CookieOptions { Expires = DateTimeOffset.UtcNow.AddDays(-1), Path = "/Lead", HttpOnly = true, Secure = true, SameSite = SameSiteMode.None });

                    // Antiforgery cookie (name from Program.cs)
                    Response.Cookies.Delete("LeadAntiforgeryCookie", new CookieOptions { Path = "/Lead", Secure = true, HttpOnly = false, SameSite = SameSiteMode.None });
                    Response.Cookies.Append("LeadAntiforgeryCookie", "", new CookieOptions { Expires = DateTimeOffset.UtcNow.AddDays(-1), Path = "/Lead", Secure = true, HttpOnly = false, SameSite = SameSiteMode.None });

                    // Session cookie (was configured SameSite=Strict)
                    Response.Cookies.Delete("LeadSession", new CookieOptions { Path = "/Lead", HttpOnly = true, Secure = true, SameSite = SameSiteMode.Strict });
                    Response.Cookies.Append("LeadSession", "", new CookieOptions { Expires = DateTimeOffset.UtcNow.AddDays(-1), Path = "/Lead", HttpOnly = true, Secure = true, SameSite = SameSiteMode.Strict });
                }
